...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchRestaurant = async () => {
      try {
        const res = await axios.get(`https://srv1070767.hstgr.cloud/api/admin/${restaurantId}/details`, {
    ...
    const fetchMenu = async () => {
      try {
        const res = await axios.get(`https://srv1070767.hstgr.cloud/api/admin/${restaurantId}/menu`, {
          headers: { Authorization: `Bearer ${token}` },
        });
...
async function batchUpdate(items, batchSize = 5) {
  ...
  while (index < items.length) {
    ...
    await Promise.all(
      batch.map(item =>
        axios.put(
          `https://srv1070767.hstgr.cloud/api/admin/${item.restaurantId}/menu/${item._id}`,
          item,
...
const addItemToList = async () => {
  ...
  try {
    await axios.post(
      `https://srv1070767.hstgr.cloud/api/admin/${restaurantId}/menu`,
      newItem,
      { headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await axios.post(
        `https://srv1070767.hstgr.cloud/api/admin/bulk`,
        itemsWithImageUrls,
        { headers: { Authorization: `Bearer ${token}` } }
      );
   ...
   const handleUpgrade = async (newLevel) => {
    try {
      const res = await axios.put(
        `https://srv1070767.hstgr.cloud/api/admin/upgrade-membership/${restaurantId}`,
        { newLevel },
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
const handleUpdate = async () => {
  ...
  inStock: itemForm.inStock // keep exact value true/false
    ...
    await axios.put(
      `https://srv1070767.hstgr.cloud/api/admin/${restaurantId}/menu/${itemForm._id}`,
      updatedItem,
      { headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://srv1070767.hstgr.cloud/api/admin/${restaurantId}/menu/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const saveAllEditedItems = async () => {
    ...
    try {
      ...
      const requests = itemsToSave.map(item =>
        axios.put(
          `https://srv1070767.hstgr.cloud/api/admin/${restaurantId}/menu/${item._id}`,
          item,
          { headers: { Authorization: `Bearer ${token}` } }
        )
      );
