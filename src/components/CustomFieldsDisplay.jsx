...
const CustomFieldsDisplay = ({ restaurantId }) => {
  ...
  useEffect(() => {
    if (!restaurantId || restaurantId.length !== 24) return;

    const fetchFields = async () => {
      try {
        const res = await fetch(`${API_BASE}/custom-fields?restaurantId=${restaurantId}`);
        if (!res.ok) return; // silently ignore bad responses
  ...
  if (!fields) return null; // render nothing if no data
    ...
    <div className="text-center space-y-4 mt-8 mb-2">
      <div className="flex justify-center items-center gap-10">
        ...
        {fields.googleReview && (
          <a
            href={
              fields.googleReview.startsWith("http")
                ? fields.googleReview
                : `https://${fields.googleReview}`
            }
            target="_blank"
            rel="noopener noreferrer"
      ...
      {fields.customLine && (
        <p className="text-gray-700 text-2xl font-semibold font-sans tracking-wide">
          {fields.customLine}
        </p>
      )}
    </div>
